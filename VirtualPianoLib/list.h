...
private:
	...
	struct node {
		...
		node(valueType x, node* prev, node* next = nullptr) : val{ x }, next{ next }, prev{ prev }{};
...
{
	auto tmp = pos.ptr;
	if (tmp == m_head) return pop_front();
	if (tmp == m_tail) return pop_back();
...
template <typename T>
void list<T>::insert(listIterator pos, valueType value)
{
	if (pos.ptr == m_head) return push_front(value);
	if (pos.ptr == nullptr) return push_back(value);
	auto tmp = new node(value, pos.ptr->prev, pos.ptr);
	if (pos.ptr->prev != nullptr) pos.ptr->prev->next = tmp;
	pos.ptr->prev = tmp;
}

template <typename T>
void list<T>::push_back(valueType value)
{
	if (m_head == nullptr) 
...
template <typename T>
void list<T>::push_front(valueType value)
{
	if (m_tail == nullptr)
